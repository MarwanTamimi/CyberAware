...
public class SecondQuiz : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (currentUserSession != null)
        {
            ...
            // You can now use currentUserSession.username as needed within your quiz logic
        }
        else
        {
        ...
        StartCoroutine(LoadQuestionsFromJSON("socialQuestion.json"));
        StartCoroutine(LoadQuestionsFromJSON("SuspiciousURLs.json"));
        StartCoroutine(LoadQuestionsFromJSON("PhishingEmails.json"));
        StartCoroutine(LoadQuestionsFromJSON("PasswordCreation.json"));

        userAnswers = new List<int>(new int[20]); 

        for (int i = 0; i < 12; i++)
        {
            userAnswers[i] = -1; // -1 indicates no answer has been selected
        }
    ...
    private void SubmitQuiz()
    {
        ...
        Destroy(gameObject);
        SceneManager.LoadScene("LastFeedBack");
    }
